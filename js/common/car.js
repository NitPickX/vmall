...
Car.prototype.addToCar = function(product){
  ...
  //1.判断是否有相同商品
  if(this.hasGoods(product.code)){
    for(var i=0; i<cartlist.length; i++){
      if(cartlist[i].code == product.code){
       //有：数量加一
       cartlist[i].number += 1
      }
    }
...
//判断购物车是否有相同商品
Car.prototype.hasGoods = function(code){
  var cartlist = this.getCar();
  //判断id是否相同，
  for(var i=0; i<cartlist.length; i++){
    if(cartlist[i].code == code){
      return i;
    }
  }
...
Car.prototype.distingush = function(code){
  var cartlist = this.getCar();
  //判断id是否相同，
  for(var i=1; i<=cartlist.length; i++){
    if(cartlist[i-1].code == code){
      return i;
    }
  }
  return false;
...
//删除购物车指定的商品
Car.prototype.delGoods = function(code){
  var cartlist = this.getCar();
  for(var i=0; i<cartlist.length; i++){
    if(cartlist[i].code == code){
      //删除当前商品
